# 麻雀 点数自動計算

牌を並べた写真を渡すと、牌を認識して点数を計算します。
**写真はどこにも送信しません。認識も計算も端末の中だけで完結します。**

使い方は 2 通りあります。

| | 動かす場所 | 用途 |
| --- | --- | --- |
| **ブラウザ版** | スマホ / PC のブラウザだけ | 卓のそばで使う。PC もサーバーも要らない |
| Python 版 | PC (ローカルサーバー / CLI) | まとめて処理したいとき、スクリプトから使いたいとき |

---

## ブラウザ版 (スマホ向け)

GitHub Pages に置いてあるページをスマホで開くだけです。インストール不要、
**一度開けば機内モードでも動きます。**

```
https://<あなたのユーザー名>.github.io/mahjong-autocalc/
```

> リポジトリの Settings → Pages で Source を **GitHub Actions** にしておくと、
> **既定ブランチへの push で自動的に公開されます** (`.github/workflows/pages.yml`)。
> ブランチ名は決め打ちしていないので、既定が `main` でも別名でも動きます。
> Actions タブの「Deploy to GitHub Pages」→「Run workflow」から手動で公開することもできます。

ホーム画面に追加しておくとアプリのように起動できます (PWA)。

### 手元で試す

```bash
npm run serve     # http://127.0.0.1:8080
```

### 1. まずは撮るだけ

**設定は要りません。** いろいろな牌のデザインから作った参照データ (約 240KB) を
同梱しているので、開いてすぐ使えます。

### 2. 読み違えたら「覚えさせる」(任意)

自分の牌を登録すると、そのセットに特化した参照データになり、さらに確実になります。
必須ではありませんが、いつも同じ牌で打つなら一度やっておくと安定します。

登録するときは次の 4 枚を撮ります。牌を**表向きに一列**に並べて真上から。

| 撮るもの | 並べる牌 |
| --- | --- |
| 萬子 | 一萬 〜 九萬 |
| 筒子 | 一筒 〜 九筒 |
| 索子 | 一索 〜 九索 |
| 字牌 | 東 南 西 北 白 發 中 |

各写真について、次の 3 手順です。**1 枚ずつ選び直す必要はありません。**

1. 写真を読み込む
2. **「並び順でまとめて指定する」→ 該当のボタン**（萬子 一〜九 など）を押す
   - 左から順に割り当てられます。右から並べた場合は「右から左の順に並べた」にチェック
   - 枠が牌とずれている / 枚数が合わない場合は、先に「手で囲む」で範囲と枚数を合わせる
3. **「この結果を覚えさせる」** を押す

4 枚ぶん繰り返せば 34 種すべて登録できます。右上の「牌の登録」で進み具合を確認できます。

登録データは端末のブラウザ内 (IndexedDB) にのみ保存されます。
普段使っていて間違いを直したときも「覚えさせる」を押せば、そのぶん賢くなります。

### 3. 和了ったら撮る

1. 和了形の 14 枚を **表向きに一列** に並べて、真上から撮る
   - 鳴きがある場合は、その 3〜4 枚を少し離して置くと別のかたまりとして認識します
   - 「カメラで撮る」でその場で撮るか、「アルバムから選ぶ」で撮影済みの写真を使えます
     (PC ではドラッグ&ドロップと貼り付けにも対応)
2. 認識結果を確認 (間違っていればタップして修正)
   - 枠が牌とずれている / 枚数が合わない場合は **「範囲を指定して読む」** を使います
     - 写真の上に出る**四角の 4 隅を、牌の列の角に合わせる**だけです。斜めから撮った
       写真でも歪みを補正して切り出します (隅をつまむと拡大鏡が出ます)
     - 枚数は ± で合わせます。等分線が牌の境目と合っているか見ながら調整してください
     - 写真が横倒しなら **↺ ↻** で回転、細かく合わせたいときは **＋ −** で拡大できます
     - 鳴きがある場合は「範囲を追加」で囲みを増やせます
3. 和了牌をタップして指定する
4. 場風・自風・ツモ/ロン・立直・ドラを画面で選ぶ
5. 「点数を計算する」

---

## Python 版 (PC)

```bash
pip install -r requirements.txt
python app.py          # http://127.0.0.1:8000
```

コマンドラインからも使えます。

```bash
python -m mahjong_autocalc.cli calibrate manzu.jpg 123456789m
python -m mahjong_autocalc.cli recognize hand.jpg
python -m mahjong_autocalc.cli score hand.jpg --win 7m --tsumo --riichi --seat 南
python -m mahjong_autocalc.cli library
```

登録データは `~/.mahjong-autocalc/tile_library.npz` に保存されます
(`MAHJONG_AUTOCALC_HOME` で変更可)。ブラウザ版とは別管理です。

---

## 撮り方のコツ

認識は写真の写り方にかなり左右されます。次を守るとほぼ失敗しません。

- **明るい場所で、影が牌にかからないように**撮る
- **真上から**撮る (斜めからでも多少は補正しますが、正面が確実)
- 牌を**一列に、向きを揃えて**並べる
- 背景は牌と色が違うもの (麻雀マット、机の面など) にする
- 牌が画面に大きく写るように寄る

---

## 精度について

**学習に使っていない 8 種類の牌デザイン**に対する実測値です。

| 条件 | 牌の正解率 |
| --- | --- |
| 同梱の参照データを使わない (規則ベースのみ) | 44% |
| **同梱の参照データあり (登録なし)** | **100%** |

規則ベースだけでは、丸や竹の描かれ方がセットごとに違うため筒子・索子の「数」を
落とします。多数のデザインから作った参照データと照合するほうが、はるかに素直に
効きます。

### この数字の限界

**すべて自作の合成画像に対する値です。** 実物の写真ではこのとおりにはなりません。

- 参照データも評価画像も同じ描画器から作っているので、実際のメーカー間の違い
  (書体、彫りの陰影、印刷か彫刻か) はこれより大きい
- 実写では照明・影・撮影角度の影響を受ける

**認識結果は目で確認してください。** 読み違えたら直して「覚えさせる」を押せば、
自分の牌に特化した参照データが積み上がり、そのぶん確実になります。
うまく切り出せない写真は「範囲を指定して読む」で 4 隅を合わせてください。

---

## 対応している役・ルール

**1翻** 立直 / 一発 / 門前清自摸和 / 断幺九 / 平和 / 一盃口 / 役牌 (白發中・場風・自風) /
嶺上開花 / 搶槓 / 海底摸月 / 河底撈魚

**2翻** ダブル立直 / 七対子 / 三色同順 / 一気通貫 / 混全帯幺九 / 対々和 / 三暗刻 /
三槓子 / 三色同刻 / 混老頭 / 小三元

**3翻** 混一色 / 純全帯幺九 / 二盃口 　**6翻** 清一色

**役満** 国士無双 / 四暗刻 / 大三元 / 字一色 / 緑一色 / 清老頭 / 四槓子 / 小四喜 /
九蓮宝燈 / 天和 / 地和
**ダブル役満** 国士無双十三面 / 四暗刻単騎 / 大四喜 / 純正九蓮宝燈

食い下がり、符計算 (平和ツモ 20 符・七対子 25 符・食い平和形の 30 符切り上げを含む)、
本場・供託、赤ドラ・裏ドラに対応しています。
同じ手牌に複数の解釈がある場合は、**一番高くなる形**を自動で選びます (高点法)。

切り替えられるローカルルール: 喰い断 / 切り上げ満貫 / 連風牌の雀頭 2符・4符 /
ダブル役満 / 役満の複合

---

## 構成

同じロジックが Python と JavaScript の 2 つあります。ブラウザ版は Python を
動かせないため移植したもので、**両方に同じテストを用意して食い違いを防いでいます**
(CI で両方走らせ、画像認識は同一の画素データで突き合わせます)。

```
mahjong_autocalc/      Python 版
  tiles.py             牌の表現と MPSZ 記法のパース
  parser.py            手牌を面子構成に分解する (複数解釈を列挙)
  yaku.py / fu.py / score.py / calculator.py    役・符・点数
  cli.py / server.py   コマンドライン / ローカル Web API
  vision/              検出・特徴量・規則ベース分類・参照ライブラリ

webapp/                ブラウザ版 (これ単体で動く)
  engine/              点数計算エンジン (tiles / parser / yaku / scoring)
  vision/
    cv.js              画像処理の基本操作 (opencv.js を使わない自前実装)
    detect.js          写真から牌の面を切り出す
    features.js        切り出した面から特徴量を作る
    glyphs.js          同梱の漢字テンプレートとの照合
    heuristic.js       規則ベースの分類 (登録前のフォールバック)
    library.js         自分の牌を覚える参照ライブラリ (IndexedDB)
    prior.js           同梱の参照データ (prior-library.bin) の読み込み
    pipeline.js        画像 → 牌の並び
  worker.js            認識を別スレッドで走らせる
  sw.js                オフライン用キャッシュ
  tests/               Node のテスト

web/                   Python 版のブラウザ UI
tools/                 テンプレート生成・検証画像レンダリング・精度計測
tests/                 Python 版のテスト
```

### 認識のしくみ

1. **切り出し** — 牌の面は「明るくて彩度が低い」ので、その領域をマスクで取り出します。
   隣り合った牌は 1 つの塊に繋がるため、牌の継ぎ目 (ほぼ全高が暗い列) を検出して分割します。
   検出は縮小した画像で行い、切り出しだけ高解像度側から行うことで速くしています。
2. **特徴量** — 牌ごとに下地の色を中央値で推定し、そこから離れた画素を「刻印」として
   分離します。しきい値を牌ごとに決めているので、照明の色かぶりに強くなっています。
3. **判別** — 「自分で登録した牌 → 同梱の参照データ → 規則ベース」の順に照合します。
   照合ベクトルは牌全体・上半分・下半分をそれぞれ正規化して連結しており、
   萬子のように「共通部分が大きく、違いが小さい」牌でも差が出るようにしています。
   一致度が低いときは採用せず次の段に譲ります (弱い一致まで拾うと、規則ベースなら
   当たっていた牌を潰して正解率が下がるため)。

### 同梱の参照データ

`tools/build_prior_library.py` が作ります。牌のデザイン (下地の色、刻印の色と
大きさ、丸や竹の描き方) をランダムに変えた 40 種類を描画し、そこから特徴ベクトルを
集めて、互いに離れたものを 1 牌あたり 8 本だけ残したものです。uint8 に量子化して
あり (情報の劣化はコサイン類似度で 0.99994)、ブラウザ版 238KB / Python 版 151KB。

```bash
python tools/build_prior_library.py            # 評価だけ
python tools/build_prior_library.py --write    # 作り直して書き出す
python tools/eval_generalization.py            # デザインをまたいだ通用度を測る
```

---

## 開発

```bash
pip install -r requirements-dev.txt
python -m pytest                                  # Python 版 (70 件)

node --test webapp/tests/engine.test.mjs          # JS 版 点数計算 (35 件)
python tools/export_raw_samples.py /tmp/mj-raw    # JS 版 画像認識の入力を用意
MJ_RAW_DIR=/tmp/mj-raw node --test webapp/tests/vision.test.mjs   # (12 件)

python tools/eval_vision.py --library             # 認識精度の計測
python tools/render_sample.py "234567m234567p33s" samples/hand.jpg
```

漢字テンプレートはリポジトリに同梱しています (`mahjong_autocalc/vision/glyphs.npz` と
`webapp/vision/glyph-data.js`)。作り直す場合は日本語フォントのある環境で:

```bash
python tools/build_glyph_templates.py
```

---

## プライバシー

ブラウザ版は写真の読み込みから計算まですべてページ内で完結し、通信は一切しません
(初回の読み込みを除く)。端末に残るのは、登録した牌の特徴ベクトルだけです。
写真そのものは保存しません。

Python 版もローカルのサーバーが処理するだけで、外部には送信しません。
